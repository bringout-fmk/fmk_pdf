  ...
  def load_font(font, encoding = nil)
    ...
    unless fontfile
      ...
      FONT_PATH.each do |path|
        #puts "hernad font search", File.join(path, base)
    ...
    #puts "hernad font, fontfile", font, fontfile
    
    if font =~ /afm/o and fontfile
        # Find the array of font widths, and put that into an object.
      puts "hernad: creating list of font widths"
      first_char  = -1
      last_char   = 0

      widths = {}
      metrics.c.each_value do |details|
        #puts "hernad metrics details:",  details
        num = details["C"]
      ...
      if false
        ...
        encoding_diff.each do |cnum, cname|
          puts "hernad diffs:", cnum, cname
          (cnum - last_char).times { widths << 0 } if cnum > last_char
          last_char = cnum
          # hernad, bilo firstchar, bilo [cname]['WX']
          # widths[cnum - first_char] = fonts.c[cname]['WX'] if metrics.c[cname]
      ...
      if fontfile =~ /\.ttf$/o
        ...
        if found
          ...
          if rl and pp and ee
            puts PDF::Writer::Lang[:ttf_licence_no_embedding] % name
      ...
      puts "hernad tmp font", tmp
      
      #hernad unedfined method basefont= for PDF:Write::Object::Font'
